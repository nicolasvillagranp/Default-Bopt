...
#BoRandom Model
from src.models import BoRandomForest, random_params_value

def test_dataset_random(X: np.ndarray, y: np.array, n_iterations: int = 5, n_initial: int = 10, seed: int = 42):
    ...
    init_params = random_params_value  # Using default hyperparameters as the center for sampling
...
from src.models import BoMLPClassifier, mlp_params_value

def test_dataset_mlp(X: np.ndarray, y: np.array, n_iterations: int = 5, n_initial: int = 10, seed: int = 42):
    # 2. Split the dataset into training and testing sets
    X_train, X_test, y_train, y_test = train_test_split(
        X, y, test_size=0.2, random_state=seed, stratify=y
    )
    ...
    # 3. Define initialization parameters for 'sample' type
    init_params = mlp_params_value  # Using default hyperparameters as the center for sampling


    optimizer_random = BoMLPClassifier(X_train, y_train)
    ...
    optimizer_sample = BoMLPClassifier(X_train, y_train)
    optimizer_sample.fit(
        n_iterations=n_iterations,
        n_initial=n_initial,
        init_type='sample',
